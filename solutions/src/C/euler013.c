...
for (int y = 49; y >= 0; y --){//loads sum of every column into carry array, everything seems to be good in debugger, tested three columns to be accurate by hand.
...
for (z = 49; z >= 1; z --){//load answer array 
    answer[k] = carry[z] % 10; //takes the 1's digit of the sum of the current column and puts it into the answer
    carry[z] /= 10; //eliminates the 1's place due to integer division
    carry[z-1] += carry[z]; //adds the two digit carry to the next column
    k--;
}

answer[k] = carry[0]; //loads the first three digits into the first element of the answer array
for (k = 0; k <= 49; k++){ //prints answer
    cout << answer[k];
}
